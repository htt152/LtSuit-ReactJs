...
import xacNhanDiem from "app/axios/bangDiem/approvedAdmin";
import tuChoiDiem from "app/axios/bangDiem/deniedAdmin";
...
import getUserAdmin from "app/axios/bangDiem/getUserAdmin";
...
export default function ConfirmPoints() {
  ...
  const returnRowsData = async (startDate, endDate) => {
    let response = await getUserAdmin();
    if (response) {
      if (response.status === 400) {
        return showAlertMessage(
          "Có lỗi trong quá trình hiển thị thông tin",
          "error"
        );
      } else if (response.status === 403) {
        return history.push("/404NotFound");
      } else {
  ...
  //Implement nút xác nhận
  const approvedChange = async () => {
    const data = [];
    const row = deepClone(rows);
    for (var i = 0; i < selected.length; i++) {
      for (var j = 0; j < rows.length; j++) {
        const getData = { email: "", id: "", status: "pending" };
        if (selected[i] === rows[j].id) {
          getData.email = rows[j].email;
          getData.id = selected[i];
          data.push(getData);
          row.splice(j, 1);
          break;
        }
    ...
    setRows(row);
    setSelected([]);
    let res = await xacNhanDiem(data);
    return res;
  };

  //Implement nút từ chối
  const deniedChange = async () => {
    ...
    setRows(row);
    setSelected([]);
    let res = await tuChoiDiem(data);
    return res;
  };

  // Sự kiện click item
                        ...
                        .map((row, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell style={{ maxWidth: "130px" }}>
                                {row.title}
                              </TableCell>
  ...
  return loading ? (
    ...
    <div className={classes.root}>
      <div className={classes.fixGroup}>
        <Button
          ...
          onClick={async () => {
            let res = await approvedChange();
            if (!res) {
              showAlertMessage("Xác nhận không thành công", "error");
            } else {
              showAlertMessage("Xác nhận thành công", "success");
              returnRowsData();
            }
          }}
        >
        ...
        <Button
          ...
          disabled={disableButton}
          onClick={async () => {
            let res = await deniedChange();
            if (!res) {
              showAlertMessage("Từ chối không thành công", "error");
            } else {
              showAlertMessage("Từ chối thành công", "success");
              returnRowsData();
            }
          }}
        >
