...
export default function ListNghiPhep() {
  ...
  useEffect(() => {
    const layListNghi = async () => {
      const response = await getListNghiPhep({ userEmail: localStorage.email });
      if (!response) {
  ...
  return loading ? (
    ...
    <Grid container>
      <Grid container spacing={1} justify="center" alignItems="center">
        ...
        <Grid item xs={3} sm={1} className="itemFullWidth">
          <Link to="/xinnghi" style={{ textDecoration: "none" }}>
            <Button
              className="btnListNp"
              variant="contained"
              size="medium"
